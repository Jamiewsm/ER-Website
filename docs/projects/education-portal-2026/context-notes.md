...
- 2026-09-09 추가 지시: 공개 웹사이트 PR #123은 사용자 수정 및 별도 공개 지시까지 배포 보류. DB·포털 준비는 계속하고 교육 migration은 기존 `public.public_notices`를 보존한다. 공지 초안은 `supabase/manual/education-public-notices.pending.sql`에 전체 주석으로 보존하며 자동 적용하지 않는다. 검증된 백업·복원 후 적용 순서는 DB→포털이며 사이트는 별도다.
- 현재 코치훈련은 2026년 10~12월 3분기이며 2027년 신규 2기와 구분한다. 수업 일정은 학생 가능 시간을 취합한 뒤 확정한다.
