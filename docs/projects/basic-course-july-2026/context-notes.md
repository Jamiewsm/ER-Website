...
### 결제 (Phase 1 — 7월 기수)
- **해외 거주자: PayPal Business 송장(invoice)** — 카드 게스트 결제 가능, 기록 남음, 즉시 시작 가능. 수수료 약 4.4%+고정 ($300 기준 약 $14/인). 정원 10명 × $300 = 최대 $3,000 규모라 PG 계약보다 합리적.
- **한국 거주자: 계좌이체** — 원화 고정가 사용 (환율 시비 방지).
- Stripe는 한국 사업자 미지원이라 제외. 규모 커지면 포트원 경유 해외카드 PG(Eximbay 등) 또는 Paddle(MoR) 검토.
- 결제 안내는 사이트에 노출하지 않고 **신청 접수 후 등록 안내 메일로 발송** (계좌·PayPal 링크를 공개 페이지에 두지 않음).
- **자리 확정 = 결제 완료 순** (선착순 기준을 명확히 하는 장치).
- 세무: 현금영수증/면세 여부는 세무사 확인 필요 (보류).

### 가격 · 얼리버드
- 정가 **$300 / ₩400,000** (원화는 고정가, 조정 가능).
- 얼리버드 **6/24(수)까지 결제 완료 시 $270 / ₩360,000** (10%).
