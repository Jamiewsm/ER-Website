<!-- Complete Enneagram KB 폴더의 진입 README — 파일 구성 + 에이전트 사용 규칙 + 빠른 검색 예시 -->
---
...
중요 — 이 산출물은 PDF 원문 전체를 그대로 전사한 파일이 아닙니다. 원문 저작권을 존중하기 위해 장문 복제 대신, AI agent 가 빠르게 검색하고 원문 위치를 찾아갈 수 있도록 요약, 진단 단서, 페이지 참조, 검색 키워드 중심으로 재구성했습니다.
...
- [complete_enneagram_kb.md](./complete_enneagram_kb.md) — 진단 테스트 설계용 타입별 핵심 구조, 본능/하위유형, countertype, 문항 설계 포인트. State vs Trait + Centers/Triads 1차 필터 포인터 포함.
- [type_pair_disambiguation.md](./type_pair_disambiguation.md) — 36개 타입쌍 감별 인덱스 + tie-breaker A/B 템플릿 (모두 작성됨, target_diagnostic_axis 컬럼 포함).
- [centers_and_triads.md](./centers_and_triads.md) — Body/Heart/Head Center + Hornevian + Harmonic triad. 진단 1차 필터 (3 질문으로 후보 1-2 type 으로 좁힘).
- [type_wings.md](./type_wings.md) — 18 wing 조합 (1w9 ~ 9w8). 핵심 변형, 행동 시그니처, 약/중/강 % 표현. 결과지에서 `7w8(50%)` 형식으로 활용.
- [instinct_stacks.md](./instinct_stacks.md) — 6 본능 스택. blind/repressed (3차) 본능 패턴 + % 강도 해석. 결과지에서 `sx(80%) so(60%) sp(10%)` 형식으로 활용.
...
2. 진단 1차 필터로 `centers_and_triads.md` 의 Center/Hornevian/Harmonic 매핑을 사용해 후보 1-2 type 으로 좁힌다.
3. wing 분석은 `type_wings.md`, 본능 스택 분석은 `instinct_stacks.md` 를 참조한다.
4. 원문 맥락이 필요하면 `source_page_index.md` 의 PDF page ref 를 따라 `/Users/Joeyswoo/Downloads/Complete_Enneagram.pdf` 를 확인한다.
5. 두 타입 점수가 비슷하면 `type_pair_disambiguation.md` 에서 해당 pair 를 찾고, `target_diagnostic_axis` 컬럼이 알려주는 axis 로 감별 질문을 설계한다.
6. 한국어 문항/결과지를 작성/검토할 때는 `korean_test_copy_guide.md` 의 30 번역 패턴 + 10 before-after 예시를 따른다.
...
본 KB 는 ER 에니어그램 테스트 발전 프로젝트의 Phase 1 (KB foundation) 산출물입니다. 진행 상태와 다음 단계는 [docs/_meta/enneagram/WORK_STATUS.md](../../../_meta/enneagram/WORK_STATUS.md), 설계 컨텍스트는 [CONTEXT.md](../../../_meta/enneagram/CONTEXT.md).
