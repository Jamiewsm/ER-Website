...
const WORD_SCREENING_BANK = [
  ...
  { type: 3, ko: '목표 지향적인|부지런한|책임감 있는|융통성 있는|효율을 중시하는|최고가 되고 싶은|인정받고 싶은|유능한|야심 있는', en: 'Goal-oriented|Diligent|Responsible|Flexible|Efficiency-minded|Wanting to be the best|Wanting recognition|Capable|Ambitious' },
  { type: 4, ko: '상상력이 풍부한|직관적인|낭만적인|자신을 가엾게 여기는|개성이 강한|생각이 깊은|속마음을 잘 드러내지 않는|아름다움에 민감한|독립적인', en: 'Imaginative|Intuitive|Romantic|Self-pitying|Individualistic|Deep-thinking|Secretive|Sensitive to beauty|Independent' },
  { type: 5, ko: '철저한|차분한|사려 깊은|예리한|학구적인|재치 있는|분석적인|인색한|호기심이 많은', en: 'Thorough|Calm|Thoughtful|Perceptive|Studious|Witty|Analytical|Stingy|Curious' },
  { type: 6, ko: '준비성 있는|예의 바른|협조적인|순종적인|전통을 따르는|충성심이 강한|의존적인|걱정이 많은|의심이 많은', en: 'Prepared|Polite|Cooperative|Obedient|Traditional|Loyal|Dependent|Worried|Suspicious' },
  { type: 7, ko: '외향적인|낙관적인|충동적인|현실을 피하려는|거리낌 없는|산만한|개방적인|쾌활한|다재다능한', en: 'Outgoing|Optimistic|Impulsive|Escapist|Uninhibited|Distractible|Open-minded|Cheerful|Versatile' },
  { type: 8, ko: '자신감 있는|주도적인|용감한|도전적인|지배하려는|끈질긴|단호한|현실적인|방어적인', en: 'Confident|A natural leader|Brave|Challenging|Dominant|Tenacious|Firm|Realistic|Defensive' },
  { type: 9, ko: '나서지 않는|양보하는|감정 기복이 적은|평화를 추구하는|중립적인|수용적인|참을성 있는|느긋한|편견 없는', en: 'Unassuming|Yielding|Emotionally steady|Peace-seeking|Neutral|Accepting|Patient|Easygoing|Unbiased' }
];
const wordScreeningQuestions = Array.from({ length: WORD_SCREENING_BANK[0].ko.split('|').length }, (_, wordIndex) =>
...
function scoreWordScreening(responses) {
  ...
  return { version: 'word-narrative-v2', responses: { ...responses }, ranked, scores: Object.fromEntries(ranked.map((row) => [row.type, row.score])), candidates, needsClarification: Boolean(reason), unclear: Boolean(reason), reason, missing };
}
...
const deep = {
  ...
  5:[{id:'d5_1',type:5,q:'문제나 상황 한가운데 뛰어들기보다, 한 걸음 물러서서 전체를 파악하고 나서야 마음이 안정되는 편이다.'},{id:'d5_2',type:5,q:'누군가 갑자기 속마음을 말해 달라고 하거나 감정적으로 의지하려 하면, 부담스러워 거리를 두는 편이다.'}],
  ...
  8:[{id:'d8_1',type:8,q:'부당하게 눌리거나 침범당한다고 느끼면, 물러서기보다 주도권을 되찾아야 한다는 반응이 먼저 올라온다.'},{id:'d8_2',type:8,q:'칭찬을 받지 못하는 것보다, 내 의견이 무시되거나 내가 함부로 대우받는 상황에 더 예민한 편이다.'}],
...
const tb56 = [
  ...
  {id:'tb_5_6_2',format:'ab',leftType:5,rightType:6,q:'둘 중 내게 더 견디기 어려운 상황은 어느 쪽에 가까운가?',a:'사람들의 연락과 요구에 계속 응하느라 혼자 쉴 시간과 기운이 없어지는 것',b:'확인이 덜 된 채 움직였다가 나중에 문제가 터지는 것'}
];
const tb58 = [
  {id:'tb_5_8_1',format:'ab',leftType:5,rightType:8,q:'다른 사람의 간섭 없이 혼자 결정하고 싶을 때, 그 이유에 더 가까운 것은?',a:'다른 사람의 요구에 시간과 기운을 빼앗기지 않고, 혼자 생각할 여유를 지키고 싶기 때문이다.',b:'남들에게 내 결정권과 주도권을 넘겨주는 것이 싫고, 누군가의 통제나 지시 아래 놓이는 상황 자체를 참을 수 없기 때문이다.'},
  {id:'tb_5_8_2',format:'ab',leftType:5,rightType:8,q:'갈등이 생겼을 때, 나를 더 화나게 하는 것은?',a:'일이 체계 없이 진행되거나 다른 사람의 실수 때문에, 내 시간과 기운을 들여 수습해야 할 때 화가 난다.',b:'누군가 나를 만만하게 보거나 힘의 불균형을 이용해 내 영역을 침범하고 통제하려 들 때 가장 화가 난다.'}
];
const tb59 = [
...
const tb35 = [
  {id:'tb_3_5_1',format:'ab',leftType:3,rightType:5,q:'내 능력을 보여 줘야 할 때, 더 두려운 상황은?',a:'노력한 만큼 성과를 내지 못해, 실패한 사람으로 평가받고 인정받지 못하는 것',b:'이 분야를 충분히 알지 못한다는 사실이 드러나, 맡은 일을 감당할 능력이 없다고 평가받는 것'},
  {id:'tb_3_5_2',format:'ab',leftType:3,rightType:5,q:'지식을 쌓고 자격증이나 성과를 얻으려고 애쓰는 이유에 더 가까운 것은?',a:'성공한 사람으로 인정받고, 내 능력을 사람들에게 보여 주고 싶기 때문이다.',b:'충분히 알지 못하면 상황을 감당하기 어렵다고 느껴, 전문성을 갖춰야 안심하고 참여할 수 있기 때문이다.'}
];
const tb37 = [
  {id:'tb_3_7_1',format:'ab',leftType:3,rightType:7,q:'내가 타인에게 유용하고 싹싹한 사람으로 인정받으려 노력할 때, 그 이유에 더 가까운 것은?',a:'나라는 사람의 가치 자체가 내가 얼마나 성과를 내고 유용한가에 달려 있다고 느끼기 때문에, 쓸모를 증명해야만 내가 가치 있는 존재로 느껴지기 때문이다.',b:'내가 유용한 사람이 되어야 사람들이 나를 좋아하고, 그래야 내가 위기에 처했을 때 도움을 받을 수 있는 사람들과의 관계와 새로운 기회를 유지하고 싶기 때문이다.'},
...
const tb45 = [
  {id:'tb_4_5_1',format:'ab',leftType:4,rightType:5,q:'혼자 있을 때 주로 무엇에 관심을 기울이는가?',a:'내 감정과 결핍이 무엇을 말하는지 오래 붙들고 느껴 보는 쪽',b:'관심 있는 주제를 체계적으로 정리하고 파고드는 쪽'},
  {id:'tb_4_5_2',format:'ab',leftType:4,rightType:5,q:'힘들거나 공허할 때 나는 주로 어떻게 반응하는가?',a:'그 감정을 더 깊이 느끼면서, 왜 이런 마음이 드는지 계속 파고든다',b:'감정에 바로 빠지기보다, 일단 한발 떨어져서 머리로 정리하려 한다'},
...
const tb68 = [
  {id:'tb_6_8_1',format:'ab',leftType:6,rightType:8,q:'누군가에게 강하게 맞설 때, 그 이유에 더 가까운 것은?',a:'상대의 반응을 보면서 믿을 만한 사람인지, 다른 의도가 있는지 확인하고 싶다.',b:'누가 힘을 쥐고 있는지 명확히 하고, 내 영역을 함부로 건드리지 못하게 경고하려는 마음이 크다.'},
  {id:'tb_6_8_2',format:'ab',leftType:6,rightType:8,q:'갈등 상황에서 강하게 맞선 뒤, 내 반응에 더 가까운 것은?',a:'내가 너무 심했나, 이 일로 나중에 문제가 생기거나 보복이 오진 않을까 하고 혼자 상황을 다시 점검하곤 한다.',b:'이제 누가 위인지 상황이 확실히 정리됐다고 느끼며, 별다른 뒤끝이나 걱정 없이 훌훌 털어낸다.'},
...
const tb89 = [
  ...
  {id:'tb_8_9_2',format:'ab',leftType:8,rightType:9,q:'피할 수 없는 갈등과 마주했을 때, 몸과 마음은 어떻게 반응하는가?',a:'오히려 에너지가 올라오고 정신이 맑아지며, 끝장을 보거나 승부를 내고 싶어진다.',b:'에너지가 급격히 빠지고 피곤해지며, 어떻게든 이 상황을 빨리 덮고 지나가고 싶어진다.'},
...
const SUBTYPE_BEHAVIOR_ITEMS = {
  1: [
    { q: '기준이 어긋난 장면을 본 뒤 실제 반응에 더 가까운 것은?', sp: '겉으로 크게 말하기보다 집에 와서 내가 놓친 것과 내 책임을 다시 점검한다.', so: '사람들이 같은 기준을 따르지 않는 것이 답답해서, 올바른 방식이 무엇인지 설명하게 된다.', sx: '관련된 사람에게 바로 지적하고, 관계가 불편해져도 고쳐야 한다고 느낀다.' },
    ...
    { q: '화가 난 뒤 주로 어떤 생각이나 행동이 이어지는가?', sp: '내가 더 잘했어야 했다는 생각이 오래 남아 스스로를 몰아붙인다.', so: '왜 사람들이 기본을 지키지 않는지 답답해하며 기준을 세우려 한다.', sx: '참다가 넘기는 것보다 즉시 고치게 만들 때 마음이 풀린다.' }
  ],
  2: [
    { q: '가까운 사람에게 도움이나 관심을 받고 싶을 때 나는 어떻게 행동하는가?', sp: '직접 부탁하기보다 애교를 부리거나 힘든 티를 내며, 상대가 먼저 챙겨 주기를 기다린다.', so: '영향력 있는 사람들과 가까워지고, 모임에서 꼭 필요한 일을 맡으려 한다.', sx: '마음에 드는 사람이 나에게 끌리도록, 그 사람에게 관심을 쏟고 내 매력을 보여 주려 한다.' },
    { q: '서운함이 생겼을 때 더 자주 나타나는 방식은?', sp: '괜찮은 척하지만 속으로는 왜 나를 챙겨주지 않는지 오래 섭섭해한다.', so: '내가 이 그룹에 얼마나 기여했는지 인정받지 못하면 힘이 빠진다.', sx: '상대가 내 마음을 몰라주면 더 자주 연락하거나 상대의 마음을 거듭 확인하려 한다.' },
    { q: '도움이 필요한 사람이 보일 때 자동으로 하는 행동은?', sp: '상대에게 기대고 싶은 마음을 숨긴 채 먼저 다정하게 챙긴다.', so: '조직에서 필요한 일을 맡고 사람들을 이어 주며, 내 영향력을 넓히려 한다.', sx: '한 사람에게 집중해 특별한 관계가 되도록 에너지를 쏟는다.' }
  ],
  3: [
    { q: '성과를 보여줘야 할 때 실제로 더 자주 하는 행동은?', sp: '티 내기보다 꾸준하고 성실하게 결과를 내서 믿을 만한 사람으로 보이려 한다.', so: '남들이 인정하는 지위나 자격, 눈에 보이는 성과를 얻으려 한다.', sx: '좋아하는 사람이나 주변 사람들이 기대하는 모습에 맞춰 외모와 행동을 바꾼다.' },
    { q: '실패하거나 부족해 보일 수 있을 때 더 가까운 반응은?', sp: '겉으로 드러내기보다 더 안정적인 결과와 책임감으로 만회하려 한다.', so: '평판이나 지위가 떨어지지 않도록 다시 성과를 내서 좋은 평가를 되찾으려 한다.', sx: '상대가 실망하지 않도록 옷차림과 태도를 신경 쓰고, 능력 있는 모습을 보여 주려 한다.' },
    { q: '인정을 얻기 위해 반복되는 행동은?', sp: '묵묵히 잘해내는 사람이라는 신뢰를 쌓는다.', so: '사람들 앞에 나서거나 좋은 실적을 보여 주며 뛰어난 사람으로 인정받으려 한다.', sx: '상대가 바라는 이상적인 모습에 맞춰 나를 바꾼다.' }
  ],
  4: [
    ...
    { q: '내게 부족한 것이 있다고 느낄 때 주로 어떻게 행동하는가?', sp: '아픈 티를 내지 않으려고 버티며, 혼자 견디는 것을 자랑스럽게 여긴다.', so: '내 부족함이나 수치심을 누군가 알아주길 바라며 표현하게 된다.', sx: '내가 밀렸다고 느끼면 상대를 이겨서 내가 부족하지 않다는 것을 보여 주려 한다.' },
    { q: '행복하거나 평온한 상황에서도 반복되는 패턴은?', sp: '겉으로는 괜찮아 보여도 혼자만의 고통을 계속 견디고 있다.', so: '다른 사람들은 부족한 것이 없어 보이는데, 나만 뭔가 부족하다고 느낀다.', sx: '내게 없는 것을 가진 사람을 보면, 부러워하는 데 그치지 않고 그 사람을 이기려 한다.' }
  ],
  5: [
    { q: '여러 사람이 동시에 연락하거나 부탁할 때 주로 어떻게 행동하는가?', sp: '혼자 있을 시간과 공간을 지키려고 연락이나 만남을 줄인다.', so: '여러 사람과 두루 어울리기보다 관심 분야가 같은 사람들과 교류한다.', sx: '대부분과는 거리를 두지만 깊이 신뢰하는 한 사람에게만 속마음을 많이 털어놓는다.' },
    { q: '에너지가 부족할 때 회복 방식은?', sp: '혼자 쉴 공간과 시간을 마련하고, 익숙한 일상으로 돌아간다.', so: '중요하게 여기는 분야를 공부하거나 탐구하면서 의미를 찾는다.', sx: '깊이 통하는 사람과 대화하거나, 내가 중요하게 여기는 생각에 몰두한다.' },
    { q: '관계에서 가까워질 때 반복되는 패턴은?', sp: '상대가 내 시간과 공간을 침범하지 않는지 먼저 본다.', so: '관심 분야나 전문 지식이 통하는 사람들과 주로 가까워진다.', sx: '절대적으로 신뢰할 수 있다고 느끼면 예상보다 깊이 몰입한다.' }
  ],
  6: [
    { q: '불안하거나 위협을 느낄 때 실제로 하는 행동은?', sp: '친절하고 위협적이지 않은 태도로 다가가, 나를 도와줄 사람들과 가까워진다.', so: '명확한 규칙, 권위, 매뉴얼을 찾아 그대로 따르며 안심한다.', sx: '두려워 보이지 않으려고 먼저 강하게 나가거나 맞선다.' },
    { q: '사람을 믿어야 할 때 더 자주 하는 행동은?', sp: '상대가 나를 지켜줄 수 있는지 따뜻한 관계를 만들어 확인한다.', so: '믿고 따를 기준이 있는지, 누가 무엇을 책임지는지 확인한다.', sx: '일부러 반대 의견을 내거나 강하게 반응하면서 상대가 믿을 만한지 확인한다.' },
    { q: '앞으로 어떻게 될지 불확실할 때 주로 무엇을 하는가?', sp: '의지할 사람을 찾고 일상에서 도움받을 방법을 마련한다.', so: '규칙과 의무를 분명히 해 모호함을 줄인다.', sx: '위협을 제거하기 위해 먼저 힘을 보여준다.' }
  ],
  7: [
    { q: '욕구와 책임이 부딪힐 때 실제로 더 자주 하는 행동은?', sp: '나와 내 사람에게 이익이 되는 기회와 정보를 빠르게 챙긴다.', so: '내가 하고 싶은 일을 미루고, 함께 정한 일정이나 다른 사람들이 기대하는 일을 먼저 한다.', sx: '소소한 재미보다 큰 감동을 주거나 이상적으로 느껴지는 사람과 일에 몰입한다.' },
    { q: '지루하거나 뭔가 부족하다고 느낄 때 무엇을 찾는가?', sp: '도움받을 사람과 필요한 정보, 새로 해 볼 수 있는 일을 찾는다.', so: '내 즐거움을 바로 챙기기보다 좋은 사람으로 남기 위해 역할을 맡는다.', sx: '일상이 시시하게 느껴지면 마음을 강하게 끄는 사람이나 새로운 가능성을 찾는다.' },
    { q: '사람들 앞에서 욕심 있어 보일 수 있을 때 반응은?', sp: '필요한 것을 확보하면서 가까운 사람들과도 나눌 방법을 생각한다.', so: '내 욕구를 앞세우는 것이 이기적으로 보일까 봐 먼저 양보하거나 참여한다.', sx: '한 대상이나 아이디어가 이상적으로 느껴지면 현실 조건보다 그 가능성에 빨려 들어간다.' }
  ],
  8: [
    { q: '누가 결정권을 가질지 중요한 상황에서 나는 어떻게 행동하는가?', sp: '크게 다투기보다 내 생활에 필요한 돈과 물건, 내가 결정할 수 있는 일을 조용히 확보한다.', so: '사람들 사이에 힘의 불균형이 보이면, 요청받지 않아도 개입하게 된다.', sx: '관계나 상황을 내가 주도하려고 내 뜻을 강하게 밀어붙인다.' },
    ...
    { q: '내 힘을 쓰는 방식에 가까운 것은?', sp: '필요한 돈이나 물건을 확보하고, 내가 원하는 것을 직접 얻으려 한다.', so: '내 힘을 과시하기보다 가까운 사람들과의 의리를 지키고 그들을 보호하는 데 쓴다.', sx: '상대에게 강렬한 인상을 남기고 관계를 주도하려 한다.' }
  ],
  9: [
    { q: '내 문제와 욕구를 미루게 될 때 실제 행동은?', sp: '먹거나 영상을 보거나 잠을 자며 몸을 편하게 하고, 복잡한 문제는 잠시 잊으려 한다.', so: '모임 일이 굴러가지 않으면 내 필요를 미루고 빈자리를 메운다.', sx: '중요한 사람의 의견과 목표를 내 것처럼 받아들이며 내 목소리를 줄인다.' },
    { q: '갈등이나 불편한 결정을 피할 때 더 가까운 방식은?', sp: '익숙한 일에 몰두하면서 불편한 감정을 잊으려 한다.', so: '바쁘게 참여하고 돕느라 내 문제를 볼 시간을 없앤다.', sx: '상대에게 맞추다 보니 내 생각을 분명히 말하지 않게 된다.' },
    { q: '에너지가 부족해도 계속 하게 되는 행동은?', sp: '몸이 편해지는 작은 습관으로 현실의 압박을 낮춘다.', so: '그룹이나 주변 사람이 필요로 하면 내 한계를 넘겨서라도 움직인다.', sx: '가까운 사람의 리듬에 맞추며 내가 먼저 해야 할 일을 미룬다.' }
  ]
};
...
const WING_BEHAVIOR_PROMPTS = {
  ...
  4: ['내 감정이 어떤지, 무엇이 아쉽고 부족한지 오래 생각했다.', '불편한 감정을 빨리 넘기기보다 그 의미를 계속 생각했다.', '평범하게 지나가는 것보다 나만의 감정을 잃는 것이 더 불편했다.'],
  ...
  7: ['실제로 했던 행동은 일이 막혔을 때 다른 방법을 찾는 쪽에 가까웠다.', '분위기가 무거워지면 기분을 바꿀 다른 일을 계획했다.', '답답한 제약이 보이면 새로운 방법이나 더 나은 선택지를 떠올렸다.'],
  8: ['실제로 했던 행동은 경계를 세우고 주도권을 되찾는 쪽에 가까웠다.', '누군가 선을 넘는다고 느끼면 직접 개입했다.', '내 의견이나 권리가 무시된다고 느끼면 더 강하게 주장했다.'],
...
function buildWingQuestionSet(core) {
  ...
  return [0, 1, 2].map((index) => ({
    ...
    q: index === 0
      ...
      : '스트레스를 받거나 마음이 불안할 때 더 자주 하는 행동은?',
    qEn: base.qEn,
...
const tb16 = [
  ...
  {id:'tb_1_6_2',format:'ab',leftType:1,rightType:6,q:'상사나 회사의 규정이 내 생각과 다를 때, 내 속마음은 어느 쪽에 더 가까운가?',a:'"저 방식은 비합리적이고 틀렸어. 기준에 맞게 제대로 뜯어고쳐야 해."',b:'"따르긴 하겠지만, 진짜 저대로 해도 안전한지 계속 의심스럽고 찜찜해."'},
...
const tb19 = [
  ...
  {id:'tb_1_9_3',format:'ab',leftType:1,rightType:9,q:'내 안에서 짜증이나 분노가 올라올 때, 나는 무의식적으로 어떻게 반응하는가?',a:'감정을 억누르려 하지만, 목소리가 굳어지거나 표정이 차가워지는 등 화난 마음이 드러난다.',b:'화난 감정 자체를 희미하게 만들려 하며, 아무 일 없는 척 멍해지거나 다른 데로 주의를 돌린다.'}
];
...
const tb26 = [
  {id:'tb_2_6_1',format:'ab',leftType:2,rightType:6,q:'다른 사람을 돕거나 챙길 때, 내 마음 깊은 곳에서 더 신경 쓰이는 쪽은 어느 것인가?',a:'내가 이 사람에게 정말 쓸모 있고 필요한 존재로 여겨지고 있을까?',b:'내가 이렇게 하면 서로 믿고 의지할 수 있는 관계가 될까?'},
...
const tb28 = [
  ...
  {id:'tb_2_8_2',format:'ab',leftType:2,rightType:8,q:'내가 아끼는 사람들을 대할 때, 내 태도는 어느 쪽에 더 가까운가?',a:'그들의 세세한 감정과 필요를 파악하고, 속마음을 나누며 가깝게 지내려 한다.',b:'그들이 억울한 일을 당하지 않게 내 울타리 안에서 확실히 보호하고 끌어주려 한다.'},
...
const tb29 = [
  ...
  {id:'tb_2_9_2',format:'ab',leftType:2,rightType:9,q:'거절이나 거리감이 생겼을 때, 무엇이 더 마음을 힘들게 하는가?',a:'내가 더 이상 상대에게 특별히 필요하지 않은 사람처럼 느껴지는 것',b:'관계 안의 불편한 긴장이 오래 이어지는 것'},
...
const tb39 = [
  {id:'tb_3_9_1',format:'ab',leftType:3,rightType:9,q:'여러 사람과 있을 때, 상황이나 분위기에 맞추는 이유는 무엇인가?',a:'이 상황에서 가장 괜찮고 유능해 보이는 역할이 뭘까?',b:'어떻게 해야 튀지 않고 무난하게 이 분위기에 스며들까?'},
  {id:'tb_3_9_2',format:'ab',leftType:3,rightType:9,q:'사람들이 나를 주목할 때, 내 안에서 드는 자연스러운 감정은?',a:'부담스러울 때도 있지만, 속으로는 내가 돋보이고 인정받는 것 같아 힘이 난다.',b:'그냥 조용히 내 자리에 있고 싶은데, 시선을 받는 것 자체가 불편하고 피곤하다.'},
...
const tb47 = [
  ...
  {id:'tb_4_7_2',format:'ab',leftType:4,rightType:7,q:'다람쥐 쳇바퀴 돌듯 반복되는 뻔한 일상 속에서 내가 더 자주 느끼는 불편함은?',a:'삶의 진짜 의미와 깊이가 사라진 것 같아 가슴 한구석이 텅 빈 것 같은 공허함',b:'더 새롭고 재밌는 일들이 많은데, 이 좁은 현실에 묶여 있는 것 같은 답답함'},
...
const tb48 = [
  {id:'tb_4_8_1',format:'ab',leftType:4,rightType:8,q:'내가 누군가에게 강하게 화를 내고 내 요구를 밀어붙일 때, 내면 깊은 곳에 깔려 있는 감정은?',a:'겉으로는 강하게 화를 내지만, 내면에는 결국 나는 이해받지 못했다는 깊은 결핍감과 남들보다 부족해 보이기 싫은 마음이 깔려 있다.',b:'상대가 선을 넘었다고 느껴, 내 권리와 주도권을 되찾기 위해 강하게 맞서야 한다고 생각한다.'},
  {id:'tb_4_8_2',format:'ab',leftType:4,rightType:8,q:'사람들과 경쟁하거나 부딪혀서 이기려 할 때, 내가 얻고 싶은 것은?',a:'나를 거절하거나 무시했던 사람들보다 내가 더 우월하다는 것을 증명하여, 내 안의 수치심과 상처를 보상받고 싶다.',b:'내 사람들과 내 영역을 지켜내고, 그 누구도 나를 함부로 통제하거나 만만하게 볼 수 없도록 힘의 우위를 확실히 하고 싶다.'}
];

// 전용 타이브레이커가 없는 31개 쌍: 동기·두려움·세계관 차이 기반 전용 질문 (키: 'typeA_typeB')
const tbCustomMap = {
  '1_2': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '사람을 도울 때, 그 사람에게 필요한 존재가 되는 것보다 무엇이 더 바르고 적절한지 바로잡아 주고 싶어지는 편이다.', b: '사람을 도울 때, 무엇이 맞는지 알려주는 것보다 그 사람에게 필요한 존재가 되고 관계가 가까워지는 쪽이 더 중요하다.' },
  '1_4': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '내 부족함을 발견하면, 그것을 고치고 바로잡아야 할 문제로 느끼는 편이다.', b: '내 부족함을 발견하면, 그 감정이 내 정체감과 깊게 연결된다고 느끼는 편이다.' },
  '1_5': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '문제나 부조리를 보면, 개선하거나 바로잡아야 한다는 긴장이 먼저 올라온다.', b: '문제나 부조리를 보면, 일단 물러나 구조를 이해하려는 쪽으로 간다.' },
  '1_6': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '무엇이 옳은지에 대한 내 기준이 행동의 출발점이 되는 편이다.', b: '무엇이 안전한지 확인하는 과정이 행동의 출발점이 되는 편이다.' },
  '1_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '불편하거나 잘못된 장면을 보면, 바로잡고 싶다는 긴장이 먼저 올라온다.', b: '불편하거나 잘못된 장면을 보면, 더 커지지 않게 지나가길 바라는 마음이 먼저 올라온다.' },
  '2_4': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '관계에서 더 중요한 것은 상대의 필요를 채우고 사랑받는 것이다.', b: '관계에서 더 중요한 것은 상대가 나를 깊고 특별하게 이해해 주는 것이다.' },
  '2_5': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '사람들과 마음을 나누며 가까워질 때 힘이 나는 편이다.', b: '사람들과 오래 어울리면 지쳐서 혼자 생각을 정리할 시간이 필요한 편이다.' },
  '2_6': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '사람들에게 잘해줄 때, 관계가 멀어지지 않도록 연결을 지키려는 마음이 더 크다.', b: '사람들에게 잘해줄 때, 신뢰를 쌓아 안전한 관계 기반을 만들려는 마음이 더 크다.' },
  '2_7': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '내 관심은 사람들의 필요와 관계에 더 자주 향한다.', b: '내 관심은 재미있는 경험과 새로운 가능성에 더 자주 향한다.' },
  '2_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '다른 사람 의견에 맞출 때, 관계가 더 가까워지길 바라는 마음이 큰 편이다.', b: '다른 사람 의견에 맞출 때, 마찰을 줄이고 조용히 넘어가려는 마음이 큰 편이다.' },
  '3_4': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '눈에 보이는 성취와 결과를 통해 내 가치를 확인하려는 편이다.', b: '내 감정의 진실함과 고유함을 통해 내가 누구인지 확인하려는 편이다.' },
  '3_5': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '지식이나 실력을 쌓을 때, 그것이 분명한 성과와 인정으로 이어지는지가 중요하다.', b: '지식이나 실력을 쌓을 때, 그것을 이해하고 익히는 과정 자체가 더 중요하다.' },
  '3_7': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '내 에너지는 목표를 이루고 결과를 만드는 쪽에 더 자주 모인다.', b: '내 에너지는 새롭고 흥미로운 가능성을 넓히는 쪽에 더 자주 모인다.' },
  '3_8': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '목표를 밀어붙일 때, 다른 사람이 나를 어떻게 평가하는지가 중요한 편이다.', b: '목표를 밀어붙일 때, 다른 사람의 평가보다 내 영향력과 통제감이 더 중요한 편이다.' },
  '3_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '일이 잘 풀리지 않으면, 실패해 보일까 봐 불안과 초조가 먼저 올라온다.', b: '일이 잘 풀리지 않으면, 갈등이 커질까 봐 불편하고 피하고 싶어진다.' },
  '4_5': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '혼자 있을 때, 내 감정과 상상 속으로 더 깊이 들어가는 편이다.', b: '혼자 있을 때, 정보와 지적 관심사 속으로 더 깊이 들어가는 편이다.' },
  '4_6': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '불안의 바닥에는 내가 남들과 다르고 어딘가 결핍되어 있다는 느낌이 깔려 있는 편이다.', b: '불안의 바닥에는 세상이 예측하기 어렵고 안전장치가 필요하다는 느낌이 깔려 있는 편이다.' },
  '4_7': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '삶의 깊이와 의미를 위해 불편한 감정도 어느 정도 붙들고 있는 편이다.', b: '삶의 즐거움과 자유를 위해 불편한 감정에서 빨리 벗어나려는 편이다.' },
  '4_8': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '상처를 받으면 내 안으로 더 깊이 들어가 그 감정을 오래 붙드는 편이다.', b: '상처를 받으면 원인을 밖에서 찾고 강하게 맞서는 쪽으로 가는 편이다.' },
  '4_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '평범하게 섞이는 것보다, 나만의 다름과 고유함이 더 중요하게 느껴진다.', b: '두드러지기보다, 무리 없이 어울리고 평온한 흐름을 유지하는 쪽이 더 중요하게 느껴진다.' },
  '5_6': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '불안할 때, 정보를 모으고 분석해 이해하려는 쪽으로 간다.', b: '불안할 때, 위험을 확인하고 대비책을 세우는 쪽으로 간다.' },
  '5_8': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '갈등이 생기면, 거리를 두고 관찰하고 분석하려는 편이다.', b: '갈등이 생기면, 직접 들어가 흐름을 바꾸고 통제하려는 편이다.' },
  '5_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '사람들과 거리를 둘 때, 내 에너지와 공간을 지키기 위한 경우가 더 많다.', b: '사람들과 거리를 둘 때, 불편한 마찰을 피하고 조용히 넘어가기 위한 경우가 더 많다.' },
  '6_7': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '불확실한 미래를 보면, 먼저 최악의 경우와 대비책을 생각하는 편이다.', b: '불확실한 미래를 보면, 먼저 더 나은 가능성과 탈출구를 떠올리는 편이다.' },
  '6_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '결정을 내리기 전, 여러 의견과 잠재 위험을 확인해야 마음이 놓인다.', b: '결정을 내릴 순간이 오면, 갈등을 피하려고 미루거나 남의 흐름을 따르기 쉬운 편이다.' },
  '7_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '불편한 상태가 오면, 더 나은 가능성이나 즐거운 계획 쪽으로 빨리 옮겨가려는 편이다.', b: '불편한 상태가 오면, 익숙하고 무난한 활동 속으로 들어가 감각을 무디게 하려는 편이다.' },
  '8_9': { q: '부담스럽거나 불편한 상황에서 평소 내 마음과 더 가까운 답을 골라 주세요.', a: '갈등은 힘을 쓰고 흐름을 바꿔야 하는 장면이 되기 쉽다.', b: '갈등은 평온과 연결을 깨뜨릴 수 있어 가능하면 피하고 싶은 장면이 되기 쉽다.' }
};
...
function renderQuestions(containerId, items, prefix) {
          ...
          <fieldset id="${prefix}-fieldset-${item.id}" aria-labelledby="${legendId}">
            ...
            <label class="block cursor-pointer">
              ...
              <div class="rounded-xl border-2 border-gray-200 p-3 text-sm text-gray-500 peer-checked:border-[#30322D] peer-checked:bg-[#FBFAF5] peer-focus-visible:ring-2 peer-focus-visible:ring-[#30322D]/30 smooth text-center">
                ${pageLang === 'en' ? 'Neither (no scoring)' : '모르겠다 / 둘 다 아니다'}
              </div>
            </label>
            </div>
          </fieldset>
      ...
      <div class="bg-white p-5 sm:p-7 rounded-xl border border-gray-100 shadow-sm" id="${prefix}-block-${item.id}">
        ...
        <div class="mt-4 text-center">
          ...
          <label for="${item.id}-U" class="inline-flex items-center rounded-full border border-gray-200 px-3 py-1.5 text-xs text-gray-500 cursor-pointer hover:bg-gray-50 peer-checked:border-[#30322D] peer-checked:bg-[#FBFAF5] peer-checked:text-[#30322D] peer-focus-visible:ring-2 peer-focus-visible:ring-[#30322D]/30 smooth">
            ${pageLang === 'en' ? 'Not sure / varies by situation (no scoring)' : '모르겠다 / 상황에 따라 다르다'}
          </label>
        </div>
        </fieldset>
      </div>`;
  ...
  if (testState.assessmentVersion === 'word-narrative-v2') bindAssessmentPager(root, items, prefix);
}
...
function getLikertResponseStats(responses, assessmentVersion) {
  if (assessmentVersion === 'word-narrative-v2') {
...
function buildResponseQualitySnapshot({ responses, timings, scoringAxes, ranked, instinctPct, confidence, assessmentVersion }) {
  ...
  if (responseStats.totalCount >= 8 && responseStats.unknownRatio >= 0.25) {
    addQualityFlag(flags, 'unknown_overuse', 'caution', '판단하기 어렵다는 답이 많음', `응답 중 ${Math.round(responseStats.unknownRatio * 100)}%가 잘 모르겠다/둘 다 아니다입니다.`);
  }
...
function maybeShowPhase4(resultData) {
  ...
  if (resultData.assessmentVersion === 'word-narrative-v2') {
...
function renderTypeElimination() {
  if (testState.assessmentVersion === 'word-narrative-v2') return renderWordScreening();
...
function getValidSavedAssessment(saved) {
  if (!saved || saved.version !== 'word-narrative-v2') return null;
  const wordResponses = {};
  wordScreeningQuestions.forEach((question) => {
...
function buildNarrativeInstinctQuestions() {
  ...
  return [
    ...
    { id: 'narrative_instinct_3', format: 'abc', instinctChoice: true, allowUnknown: true,
      q: '바쁜 한 주를 보낸 뒤, 무엇이 부족했다고 느낄 때 가장 먼저 채우고 싶은가?', qEn: 'After a busy week, which missing part do you most want to restore?',
      options: [
        { value: 'so', inst: 'so', text: '사람들과 어울리고 소속감을 느끼는 시간. 모임 소식이나 사람들과의 연락을 놓치고 있지는 않은지 살핀다.', textEn: 'A sense of connection with people. I check whether I have lost touch with groups and relationships.' },
        { value: 'sp', inst: 'sp', text: '몸을 돌보고 생활을 챙기는 시간. 잠과 식사, 지출, 혼자 쉴 시간부터 점검한다.', textEn: 'A stable body and daily life. I first attend to sleep, food, money, or time to rest.' },
        { value: 'sx', inst: 'sx', text: '마음이 뜨거워지는 경험. 나를 끌어당기는 사람이나 일에 푹 빠지고 싶다.', textEn: 'A sense of intensity and aliveness. I want deep engagement with a person or pursuit that moves me.' }
      ] }
  ];
}
  ...
  types.forEach((leftType, leftIndex) => types.slice(leftIndex + 1).forEach((rightType) => {
    ...
    for (let index = 0; index < 2; index += 1) {
      ...
      pairs.push({
        ...
        q: source ? getQuestionText(source) : index === 0 && custom ? custom.q : '불편했던 경험을 떠올려 보세요. 그때 그렇게 행동한 이유에 더 가까운 쪽은?',
...
function submitNarrativePhase() {
  ...
  if (ranked[0].score > 0 && ranked[0].score - ranked[1].score <= 1 && ranked[1].score > ranked[2].score) {
    testState.phase3Question = { ...buildPostTieQuestion(ranked[0].type, ranked[1].type), q: '두 설명이 모두 나와 비슷하다면, 평소 행동의 이유에 더 가까운 쪽은?', qEn: 'When both sound familiar, which inner concern has driven you more often?' };
...
function submitTypeElimination() {
  if (testState.assessmentVersion === 'word-narrative-v2') return submitWordScreening();
...
function submitPhase1() {
  if (testState.assessmentVersion === 'word-narrative-v2') return submitCandidateClarification();
...
function submitPhase2() {
  if (testState.assessmentVersion === 'word-narrative-v2') return submitNarrativePhase();
  if (!validate(testState.phase2Questions, 'p2', 'validation-msg-2')) return;
...
function submitPhase3() {
  if (testState.assessmentVersion === 'word-narrative-v2') return submitNarrativeTieBreak();
...
function renderGuidedApplicationSection(model) {
  ...
  const applicationMap = [
    {
      label: '중요한 것',
      title: '내가 지키려는 것',
      body: `${coreName}의 반복 패턴은 약점보다 먼저, 내가 무엇을 중요하게 여기고 원하는지 보여줍니다.`,
      value: protectedNeed
    },
    {
      label: '강점',
      title: '건강할 때 드러나는 강점',
      body: '좋은 상태에서 이 성향은 일과 관계에서 도움이 되는 강점으로 나타납니다.',
      value: careerStrength
    },
    {
      label: '스트레스',
      title: '압박에서 과해지는 것',
      body: '스트레스가 커질수록 평소 강점이 지나치게 나타나, 가까운 사람에게 부담을 줄 수 있습니다.',
      value: pressurePattern
    },
    {
      label: '주변의 도움',
      title: '가까운 사람이 알아야 할 것',
      body: '나를 고치려 하기보다, 내가 왜 그렇게 반응하는지 함께 살펴볼 때 변화가 시작됩니다.',
  ...
  const steps = [
    {
      label: '나',
      title: '나를 이해하는 단계',
      body: `${coreName}의 핵심 동기와 방어적인 반응을 이해하면, 내가 비슷한 상황에서 왜 같은 선택을 하는지 살펴볼 수 있습니다.`,
      bullets: [protectedNeed, pressurePattern]
    },
    {
      label: '주변 사람',
      title: '타인을 이해하는 단계',
      body: '에니어그램의 실제 가치는 배우자·자녀·동료가 무엇을 지키려 하고 어떤 상황을 힘들어하는지 이해하는 데서 시작됩니다.',
    ...
    },
    {
      ...
      bullets: [counselingFocus, '가족·팀·사역 현장에서 꾸준히 실천할 방법 정하기']
    }
  ];
  ...
  return `
    <section id="report-application" class="er-report-section er-report-application">
      <div class="er-report-section-head">
        <span>일상에 적용하기</span>
        <h2>나와 주변 사람에게 적용하기</h2>
      </div>
      <div class="er-report-application-hero">
        <p>결과를 읽으며 내 성향과 바라는 것, 강점을 살펴보세요. 주변 사람과 다른 점을 이해하면, 대화하거나 도움을 줄 때 더 잘 맞는 방법을 찾을 수 있습니다.</p>
      </div>
...
const CONSULTATION_QUESTION_MAP = {
  ...
  '3_6': [
    '압박을 받으면 빨리 성과를 내야 마음이 놓이나요, 빠진 위험을 확인해야 안정되나요?',
    '실패 가능성이 보일 때 무능해 보일까 봐 긴장하나요, 대비가 부족할까 봐 긴장하나요?'
  ],
  '3_9': [
    '상대에게 맞추는 이유가 좋은 평가를 받기 위해서인가요, 갈등을 피하기 위해서인가요?',
    '압박을 받으면 더 열심히 하려고 하나요, 일단 미루고 쉬고 싶어지나요?'
  ],
  '4_7': [
  ...
  '5_9': [
    '물러나는 이유가 에너지 보존인가요, 관계의 긴장을 줄이려는 것인가요?',
    '침묵할 때 내 공간을 지키는 느낌인가요, 갈등을 덮고 넘어가려는 느낌인가요?'
  ],
  '6_8': [
  ...
  '7_9': [
    '불편함을 피할 때 다른 가능성으로 전환하나요, 익숙한 일에 몰두하며 문제를 미루나요?',
...
function renderExecutiveSummaryCard(model) {
  ...
  const wingLine = model.selectedWing
    ? `${model.selectedWing}번의 성향이 핵심 유형의 행동 방식에 함께 나타날 수 있습니다.`
    : '두 날개 중 어느 쪽이 더 가까운지 추가로 살펴볼 필요가 있습니다.';
  ...
  return `
    <section id="report-executive" class="er-report-executive" aria-label="결과 핵심 요약">
      <div class="er-report-executive-head">
        ...
        <p>이 결과를 평소 내 모습과 비교해 보세요. 잘 맞지 않거나 헷갈리는 부분은 상담에서 함께 살펴볼 수 있습니다.</p>
      </div>
...
function buildReportEvidenceCopy(model) {
  ...
  const topLine = top && second && Number.isFinite(topShare) && Number.isFinite(secondShare)
    ? `${top.type}번의 점수가 가장 높고, ${second.type}번이 그다음으로 나타났습니다. 이 차이는 결과를 단정하기보다 실제 삶의 반복 장면에서 함께 확인할 근거입니다.`
    : '점수는 유형 이름을 확정하는 숫자가 아니라, 평소 어떤 마음으로 반응하는지 보여줍니다.';

  return {
    topLine,
    instinctLine: synthesis.instinctSignal || '본능 점수는 어떤 생활 영역에 관심을 더 기울이는지 보여줍니다.',
...
function buildPremiumReportModel(resultData) {
  ...
  const subtypePending = resultData.assessmentVersion === 'word-narrative-v2' && !(resultData.phase4 && resultData.phase4.subtypeCode);
  ...
  const instinctRows = (resultData.assessmentVersion === 'word-narrative-v2'
    ? (resultData.instinctMetrics || [])
    : normalizeMetricRows(resultData.instinctMetrics || [])).map((row, index) => ({
    ...row,
    active: resultData.assessmentVersion === 'word-narrative-v2' ? row.code === subtypeCode : index === 0 || row.code === subtypeCode
  }));
  ...
  if (resultData.assessmentVersion === 'word-narrative-v2') {
    confidenceExplanation.label = resultData.coreResolved ? '가장 가까운 유형을 찾았습니다' : '핵심 동기를 더 확인해 주세요';
  ...
  return {
    ...
    display: {
      ...
      confidence: resultData.assessmentVersion === 'word-narrative-v2' ? (resultData.coreResolved ? '자기이해를 위한 잠정 결과' : '추가 확인 필요') : getReportConfidenceDisplay(resultData.confidence),
...
function renderPremiumReport(model) {
  ...
  const actionRows = (c.actionPlan || []).slice(0, 2).map((item, index) => `
    <label class="er-report-action-row">
      <input type="checkbox" data-report-action="${index}" aria-label="오늘의 실천 ${index + 1}">
    ...
    <article class="er-premium-report" data-report-key="${escapeReportHtml(model.reportKey)}" data-core-tone="${escapeReportHtml(model.coreTone)}">
      ...
      <section id="report-evidence" class="er-report-section er-report-visuals">
        ...
        <div class="er-report-panel">
          ...
          <p class="er-report-microcopy">상위 세 유형의 점수 합에서 각 유형이 차지하는 비율입니다. 해당 유형일 확률은 아닙니다.</p>
      ...
      <section id="report-pattern" class="er-report-section">
        ...
        <div class="er-report-formation">
          <h3 id="analysis-report-title">이 반응이 익숙해진 배경</h3>
    ...
    <article class="er-premium-report er-report-gate">
      <section class="er-report-section er-report-gate-card">
        ...
        <div class="er-report-gate-session">
          <h3>유형 상담에서 함께 살펴보세요</h3>
          <p>유형 상담에서는 사전 설문과 1:1 대화를 통해 핵심 유형, 하위유형, 날개를 함께 살펴봅니다. 검사만으로 판단하기 어렵다면 실제 경험을 이야기하며 확인할 수 있습니다.</p>
